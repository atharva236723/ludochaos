...
`diceCount` defaults to `1` from `initMenu()` (runs on page load), so the snippet above works without setting it. Pass `diceCount = 2` before `startGame()` to test double-dice mode.
...
**2-player dual-house mode** (`twoPlayerHouseMode === 1`): each human player controls two diagonal houses per turn. `G.dualHouse = true`; `G.turnColors` has only two entries (`['green','yellow']`); `G.active` still has all four colors. `curTeamColors()` (`helpers.js`) returns `[currentColor(), partner]` instead of `[currentColor()]` — every place that needs "all colors this player can move" must call `curTeamColors()`, not just `currentColor()`.

**Double-dice state on `G`** (only meaningful when `G.diceCount === 2`): `G.dicePool` (`[{v, used}]`) holds the two rolled values; `G.selSlot` tracks which slot the player has selected to spend; `G.sixPlusN` is set when one die is 6 (unlock) and the other provides the move distance; `G.turnExtra` flags a bonus roll earned this turn; `G.doubleStreak` counts consecutive doubles (three forfeits the turn); `G.lockedTid` is the Chaos killer pin that can't move on its bonus turn.
...
`js/config.js` holds the static geometry: the 52-cell `TRACK` array traced clockwise, the `SAFE` index set (8 safe squares), and per-color `start`/`home`/`base` coordinates in `PLAYERS`. (`CHAOS_TILES`/`CHAOS_SAFE` appear in design docs but are **not present in the codebase** — special chaos-tile events are not yet implemented.)
...
**Player identity cards** are rendered by `renderPlayerCorners()` (`js/render.js`) into `#playerSideLeft` and `#playerSideRight` — flex columns that flank the board. `#playerSideLeft` holds green (top) + red (bottom); `#playerSideRight` holds yellow (top) + blue (bottom). Vertical alignment is handled by CSS `space-between`. Die positions are declared in `DIE_CORNER = {green:[-1,2], yellow:[-1,13], blue:[16,13], red:[16,2]}` (board-local row/col); `DIE_PAD = 2.5` is the vertical padding offset that matches `.boardZone`'s `padding: calc(var(--cell)*2.5) 0`, placing each die flush with the board's outer edge. The `--cell` formula denominator in `css/base.css` is `20` (= 15 board cells + 2 × 2.5 padding) — keep it in sync with `.boardZone` padding. `renderPlayerCorners()` is called from `startGame()` after `render()`, and again from `_onlineStartGame()` after overriding `G.playerProfiles`.
...
- **Chaos effects** (not yet implemented): the design calls for boost / teleport / trap / swap events when a token lands on a special chaos tile, but neither `CHAOS_TILES` nor `applyChaos` exist in the current codebase. Chaos mode currently only differs from Classic via the global capture-before-home-entry gate.
...
- **Profile page** (`js/profile.js`): chrome-layer script loaded only on `profile.html`. Wires all preference controls (name, avatar, emoji, accent, theme, AI level, sound, stats) to `Prefs` via a staged `draft` object — changes preview live but only commit on "Save changes". Also handles Google sign-out (clears `Prefs.pfpDataUrl` when `session.provider === 'google'`, preserves it for local uploads) and stats reset. Never loads on any other page.
